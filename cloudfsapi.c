                              ...
                              void* dir_entry)
{
  ...
  {
    /*
      No need to capture this here, already done during connect
  ...
  for (tries = 0; tries < REQUEST_RETRIES; tries++)
  {
    ...
    else
    {
          ...
          || !strcasecmp(method, "MKDIR"))
      {
        ...
        debugf(DBG_LEVEL_EXTALL, "send_request_size: not sending meta (%s)",
               orig_path);
    }
    ...
    else if (!strcasecmp(method, "GET"))
    {
      ...
      else
      {
        ...
        debugf(DBG_LEVEL_EXT, "send_request_size: GET HEADERS only");
        curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION, &header_get_meta_dispatch);
        curl_easy_setopt(curl, CURLOPT_HEADERDATA, (void*)de);
        raw_xattrs = true;
...
int cloudfs_object_read_fp(const char* path, FILE* fp)
{
  ...
  if (flen >= options->segment_above)
  {
    ...
    curl_slist* headers = NULL;
    add_header(&headers, "item", manifest);
...
int cloudfs_list_directory(const char* path, dir_entry** dir_list)
{
  ...
  *dir_list = NULL;
    ...
    for (onode = root_element->children; onode; onode = onode->next)
    {
      ...
      if (is_object || is_container || is_subdir)
      {
        ...
        *dir_list = de;
        ...
        debugf(DBG_LEVEL_EXT,
               KCYN"new dir_entry %s size=%d %s dir=%d lnk=%d xattrs=[%p] mod=[%s]",
               de->full_name, de->size, de->content_type, de->isdir, de->islink, de->xattrs,
               time_str);
      }
      else
  ...
  {
    entry_count = 1;
    debugf(DBG_LEVEL_NORM, "Init cache entry container=[%s]",
           options->public_container);
    dir_entry* de = init_dir_entry();
    de->name = strdup(options->public_container);
